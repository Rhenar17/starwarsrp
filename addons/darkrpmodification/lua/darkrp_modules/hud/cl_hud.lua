...
surface.CreateFont("bfrontagenda", {font = "neptune", size = ScreenScale(5), weight = 400, antialias = true})
...
local recompute = matmeta.Recompute
...
local function drawblur(x, y, width, height, amount, passes)
    amount = amount or 5
    surface_SetMaterial(blur)
    drawcolor(255, 255, 255)

    for i = -(passes or 0.1), 1, 0.2 do
        setfloat(blur, "$blur", i * amount)
        recompute(blur)
        render_UpdateScreenEffectTexture()

        render_SetScissorRect(x, y, x+width, y+height, true)
...
local function Agenda()
    ...
    drawblur(10, 10, 460, 150, 5, -0.2)
    surface.SetDrawColor(0, 0, 0, 150)
    surface.DrawRect(10, 10, 460, 150)
    surface.DrawRect(10, 10, 460, 20)

    draw.DrawNonParsedText(agenda.Title:upper(), "bfrontagenda", 30, 12, colors.red, 0)
    draw.DrawNonParsedText(agendaText:upper(), "bfrontagenda", 30, 35, colors.white, 0)
end

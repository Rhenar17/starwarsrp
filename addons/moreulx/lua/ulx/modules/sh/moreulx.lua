...
function ulx.give(calling_ply,target_plys,weapon)
	...
	for i=1, #target_plys do
		local v = target_plys[ i ]
		if v:HasWeapon(weapon) then continue end
		v:Give(weapon)
		table.insert( affected_plys, v )
	end
	ulx.fancyLogAdmin( calling_ply, "#A gave "..weapon.." to #T", affected_plys )
end
local give = ulx.command("MoreULX", "ulx give", ulx.give, "!give")
give:addParam{ type=ULib.cmds.PlayersArg }
give:addParam{ type=ULib.cmds.StringArg, hint="weapon class, not name",  ULib.cmds.takeRestOfLine, completes=ulx.common_kick_reasons }
give:defaultAccess( ULib.ACCESS_ADMIN )
give:help( "Gives the player(s) weapons." )